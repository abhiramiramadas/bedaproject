...
# Role Passwords - change in config.py
try:
    from config import ROLE_PASSWORDS
except ImportError:
    ROLE_PASSWORDS = {"admin": "admin@123", "police": "police@123", "ambulance": "amb@123"}
...
class VehicleTracker:
    ...
    def update(self, boxes, frame_num):
        ...
        for box in boxes:
            ...
            for track_id, history in self.tracks.items():
                if track_id in used_tracks or not history:
                    continue
    ...
    def _cleanup(self, current_frame):
        tracks_to_remove = [
            tid for tid, history in self.tracks.items()
            if history and current_frame - history[-1][0] > self.max_age
        ]
...
class SeverityCalculator:
    ...
    def calculate(iou, speed1, speed2, vehicle_types):
        ...
        type_score = min(sum(10 for v in vehicle_types if v in heavy_vehicles), 20)
...
class DamageEstimator:
    ...
    def estimate(severity, vehicle_types, impact_score):
        base_costs = {"CRITICAL": 15000, "HIGH": 10000, "MEDIUM": 5000, "LOW": 2000}
        type_multipliers = {"Car": 1.0, "Motorcycle": 0.5, "Bicycle": 0.2, "Bus": 2.5, "Truck": 2.0}

        base_cost = base_costs.get(severity, 5000)
        multiplier = sum(type_multipliers.get(v, 1.0) for v in vehicle_types) / max(len(vehicle_types), 1)
        estimated_cost = int(base_cost * multiplier * (impact_score / 50))

        repair_days = {"CRITICAL": "30-45", "HIGH": "20-30", "MEDIUM": "10-20", "LOW": "5-10"}
...
class AccidentDetector:
    ...
    def process_frame(self, frame, fps=30):
        ...
        for i in range(len(tracked)):
            for j in range(i + 1, len(tracked)):
                track1, track2 = tracked[i], tracked[j]
                box1, box2 = track1[1][:4], track2[1][:4]
                speed1, speed2 = track1[2], track2[2]
                class1, class2 = track1[3], track2[3]
                ...
                if iou > IOU_THRESHOLD_LOW and max(speed1, speed2) >= min_speed:
                    vehicle_types = [
    ...
    def _send_alerts(self, accident_data, image_path):
        ...
        Insurance CLAIM is NOT sent automatically. Admin must
        review and trigger it manually from the dashboard.
        """
        try:
            ...
            # Step 3: Insurance evidence notice — preserve evidence,
            #          inform insurer an incident occurred.
            #          This is NOT a claim. Admin triggers claim later.
            ...
            # Step 4: Blood donation — only critical
            ...
            # Step 5: Organ donation — only critical
            if accident_data['severity'] == 'CRITICAL':
    ...
    def process_video(self, video_path, display=True, save_output=False):
        ...
        logger.info(f"Processing video: {video_path} | {width}x{height} | FPS: {fps} | Frames: {total_frames}")
...
def initiate_insurance_claim():
    ...
    Requires human approval — NOT triggered automatically.
    POST body: { "accident_index": 0 }
    """
    data = request.get_json()
    ...
    if accident.get('claim_submitted'):
        return jsonify({"error": "Claim already submitted", "claim_ref": accident.get('claim_ref')}), 400
